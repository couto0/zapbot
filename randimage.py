...
def getBackgroundColor(tempInfo) :
    if tempInfo['background'] is 'w' :
        return (255, 255, 255, 0)

def getBackground(tempInfo, template) :
    # If we're going not going to paste over image, we make a b/w layer
    if tempInfo['background'] != 'o' :
        return Image.new('RGB', template.size, getBackgroundColor(tempInfo))
    else :
        return template

def getUniqueSources(tempInfo, source_dir, temp_num, memes_dir) :
    """
...
def make_meme(tempInfo, source_dir, source_imgs, bg, template) :
    n = 0
    for box in tempInfo['boxes'] :
...
def generate():
    ...
    bg = getBackground(tempInfo, template)

    source_imgs, filename = getUniqueSources(tempInfo, source_dir, temp_num, memes_dir)

    meme = make_meme(tempInfo, source_dir, source_imgs, bg, template)
